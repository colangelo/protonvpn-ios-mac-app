...
| B | **Built 2026-09-11 as one ladder with C (#3 #4); live test pending.** The local agent not connected for 60 s while NE says Connected → restart the same connection. Design `docs/2026-09-11-liveness-ladder-design.md`; kill switch `defaults write io.github.colangelo.protonvpn.mac LivenessEnabled -bool false`; log lines `[liveness] …`. | `LegacyCommon/Core/Liveness/` (`TunnelLivenessSupervisor` + tests), `Core/VpnGateway+Liveness.swift`, one line in `VpnManager+LocalAgent.didChangeState` |
| C | **Built 2026-09-11 (same ladder); live test pending.** Still dead 60 s after the restart → re-run the saved request excluding the dead *node* (`Logical.domain`; 30-min avoid list; pinned → its country → Quick Connect; gateways never widen), at most twice. Proof: `just liveness-falsifier` (attended: two clicks + sudo). | `VpnServerSelector.selectServer(…, excluding:)`, `Core/Liveness/LivenessReselection.swift`, `VpnGateway` (`selectServerForConnect`, `autoConnect` flag) |
